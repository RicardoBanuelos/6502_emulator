...
TEST(instructions, ldx_test_immediate)
{
    for(int i = 0; i < MAX_ITERATIONS; ++i)
    {
        ...
        LDX ins(cpu, AddressingMode::Immediate, 2);
        ins.run();

        ASSERT_EQ(cpu->getRegister(Register::X), expected);
...
TEST(instructions, ldx_test_zeroPage)
{
    for(int i = 0; i < MAX_ITERATIONS; ++i)
    {
        uint16_t currentAddr = rand() % UINT16_MAX;
        ...
        LDX ins(cpu, AddressingMode::ZeroPage, 2);
        ins.run();
...
TEST(instructions, ldx_test_zeroPageY)
{
    for(int i = 0; i < MAX_ITERATIONS; ++i)
    {
        uint16_t currentAddr = rand() % UINT16_MAX;
        ...
        LDX ins(cpu, AddressingMode::ZeroPageY, 2);
        ins.run();
...
TEST(instructions, ldx_test_absolute)
{
    for(int i = 0; i < MAX_ITERATIONS; ++i)
    {
        uint16_t currentAddr = rand() % UINT16_MAX;
        ...
        LDX ins(cpu, AddressingMode::Absolute, 2);
        ins.run();
...
TEST(instructions, ldx_test_absoluteY)
{
    for(int i = 0; i < MAX_ITERATIONS; ++i)
    {
        uint16_t currentAddr = rand() % UINT16_MAX;
        ...
        LDX ins(cpu, AddressingMode::AbsoluteOffsetY, 2);
        ins.run();

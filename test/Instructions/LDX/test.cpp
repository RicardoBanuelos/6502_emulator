...
// Helper to verify LDX results
void verifyLDXResults(uint8_t value) {
    // Check X register
    ASSERT_EQ(cpu->getRegister(Register::X), value)
        << "X register should contain loaded value: expected=0x" << std::hex << (int)value;
...
uint16_t setupAddressingMode(AddressingMode mode, uint8_t testValue) {
    ...
    switch(mode) {
        ...
        default: {
            ADD_FAILURE() << "Unsupported addressing mode for LDX";
            return 0;
        }
    }
...
// Test case for LDX using different addressing modes
class LDXAddressingModeTest : public ::testing::TestWithParam<std::tuple<AddressingMode, uint8_t>> {
protected:
    void SetUp() override {
        setup();
    }
...
TEST_P(LDXAddressingModeTest, LoadXRegister) {
    auto [mode, testValue] = GetParam();
    
    std::stringstream ss;
    ss << "Testing LDX with addressing mode: ";
    switch(mode) {
        case AddressingMode::Immediate: ss << "Immediate"; break;
        case AddressingMode::ZeroPage: ss << "ZeroPage"; break;
        case AddressingMode::ZeroPageY: ss << "ZeroPageY"; break;
    ...
    // Execute LDX with the appropriate addressing mode
    std::unique_ptr<LDX> ldx(new LDX(cpu, mode, getCycleCount(mode)));
    ldx->run();
    
    // Verify results
    verifyLDXResults(testValue);
}
...
INSTANTIATE_TEST_SUITE_P(
    LDX,
    LDXAddressingModeTest,
    ::testing::ValuesIn(generateTestCases()),
    GetTestName
);

// Test page boundary crossing for AbsoluteY mode
TEST(LDXTest, PageCrossing) {
    ...
    std::unique_ptr<LDX> ldx(new LDX(cpu, AddressingMode::AbsoluteY, 5)); // 5 cycles with page crossing
    ldx->run();
    
    verifyLDXResults(0x42);
}

// Test zero page wrap-around behavior for ZeroPageY
TEST(LDXTest, ZeroPageWrapAround) {
    ...
    std::unique_ptr<LDX> ldx(new LDX(cpu, AddressingMode::ZeroPageY, 4));
    ldx->run();
    
    verifyLDXResults(0x42);
}

// Test that LDX only affects X, Z, and N flags
TEST(LDXTest, OnlyAffectsXZNFlags) {
    setup();
    ...
    // Set A and Y registers to check they're not affected
    cpu->setRegister(Register::A, 0xAA);
    cpu->setRegister(Register::Y, 0xBB);
    
    // Setup LDX Immediate with value 0x80 (negative)
    uint16_t pc = 0x1000;
    cpu->setRegister(Register::PC, pc);
    mem->writeByte(pc, 0x80);
    
    std::unique_ptr<LDX> ldx(new LDX(cpu, AddressingMode::Immediate, 2));
    ldx->run();
    ...
    ASSERT_EQ(cpu->getRegister(Register::Y), 0xBB);
    ...
    ldx.reset(new LDX(cpu, AddressingMode::Immediate, 2));
    ldx->run();
...
TEST(LDXTest, RandomizedValues) {
    ...
    for (int i = 0; i < ITERATIONS; ++i) {
        ...
        switch(modeInt) {
            ...
            case 2: mode = AddressingMode::ZeroPageY; break;
            case 3: mode = AddressingMode::Absolute; break;
            default: mode = AddressingMode::AbsoluteY; break;
        }
        
        // Random test value
        uint8_t testValue = rand() & 0xFF;
        
        // Setup memory and execute LDX
        setupAddressingMode(mode, testValue);

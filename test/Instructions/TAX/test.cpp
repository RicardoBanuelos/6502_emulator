...
TEST_F(TAXTest, BasicFunctionality) {
    for (uint8_t testValue : testValues) {
        ...
        // Execute TAX
        std::unique_ptr<TAX> tax(new TAX(cpu));
        tax->run();
        
        // Verify X now equals A
        ASSERT_EQ(cpu->getRegister(Register::X), testValue) << "X should equal A after TAX";
...
TEST_F(TAXTest, PreserveYRegister) {
    for (int i = 0; i < 10; i++) {
        ...
        ASSERT_EQ(cpu->getRegister(Register::Y), y) << "Y register should not be affected by TAX";
    }
}

// Test Zero flag behavior
TEST_F(TAXTest, ZeroFlagBehavior) {
    // Test when A is zero
    {
        SCOPED_TRACE("A = 0x00");
        SetUp();
...
TEST_F(TAXTest, NegativeFlagBehavior) {
...
// Test that TAX preserves other flags (C, I, D, B, V)
TEST_F(TAXTest, PreserveOtherFlags) {
    ...
    for (Flag flag : flagsToTest) {
        // Test with flag initially set
        {
            ...
            ASSERT_TRUE(cpu->getFlag(flag)) << "Flag should remain set after TAX";
        }
        ...
        // Test with flag initially clear
        {
            ...
            ASSERT_FALSE(cpu->getFlag(flag)) << "Flag should remain clear after TAX";
        }
    }
}
...
TEST_F(TAXTest, RandomizedBehavior) {
    ...
    for (int i = 0; i < ITERATIONS; i++) {
        ...
        // Verify X equals A
        ASSERT_EQ(cpu->getRegister(Register::X), a) << "X should equal A after TAX";

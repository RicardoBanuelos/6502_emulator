...
// Capture flags before STY operation
struct FlagState {
    bool C, Z, I, D, B, V, N;
...
class STYTest : public ::testing::TestWithParam<AddressingMode> {
protected:
    void SetUp() override {
        setup();
    }
    
    // Test values for Y register
...
uint16_t setupAddressing(AddressingMode mode) {
    ...
    switch (mode) {
        ...
        case AddressingMode::ZeroPageX:  // STY uses X indexing
            setX(0x05);
...
TEST_P(STYTest, StoreAllTestValues) {
    ...
    for (uint8_t testValue : testValues) {
        SCOPED_TRACE("Mode: " + modeName + ", Y: 0x" + 
                    (testValue < 16 ? "0" : "") + 
        ...
        // Create and run the STY instruction
        std::unique_ptr<STY> sty(new STY(cpu, mode, getCycleCount(mode)));
        sty->run();
        
        // Verify the value was stored correctly
        ASSERT_EQ(mem->readByte(effectiveAddr), testValue)
            << "Failed to store Y register value to memory";
...
TEST_F(STYTest, ZeroPageWrapAround) {
    setup();
    setY(0x42);
    setX(0xFF);
    setPC(0x0200);
    ...
    // Create and run the STY instruction with ZeroPageX addressing
    std::unique_ptr<STY> sty(new STY(cpu, AddressingMode::ZeroPageX, 4));
    sty->run();
...
// Test that STY doesn't modify any registers
TEST_F(STYTest, PreserveRegisters) {
    for (int i = 0; i < 10; i++) {
        setup();
        ...
        // Run instruction
        std::unique_ptr<STY> sty(new STY(cpu, AddressingMode::ZeroPage, 3));
        sty->run();
...
// Test with different Y values on all addressing modes
TEST_F(STYTest, RandomizedYValues) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        for (AddressingMode mode : modes) {
            ...
            ASSERT_EQ(mem->readByte(effectiveAddr), randomY) 
                << "Failed to store random Y value using addressing mode " 
...
// Generate a vector of all addressing modes used by STY
